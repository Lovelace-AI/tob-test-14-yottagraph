...
The Entity Lookup feature uses `useElementalClient` from `@yottagraph-app/elemental-api`:
...
- Using `useElementalClient` from `@yottagraph-app/elemental-api/client`
...
import type { GetNEIDResponse } from '@yottagraph-app/elemental-api/client';
...
- `useElementalClient` - For making authenticated API calls (`@yottagraph-app/elemental-api`)
